...
static void str_reservation(FILE * file,void * reservation,int * n, char F){
    ...
    double rating = get_reservationRATING(reservation);
    ...
    if(F){
                ...
                snprintf(result,1000,
                            ...
                            "rating: %d\n"
                            "total_price: %.3f\n",
                            (*n)+1,id,begin,end,user,(int)rating,total_price);
    ...
    snprintf(result,100,"%s;%s;%s;%s;%d;%.3f\n",id,begin,end,user,(int)rating,total_price);
